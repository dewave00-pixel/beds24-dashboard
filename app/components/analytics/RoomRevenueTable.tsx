        ...
        <div className="bg-white dark:bg-slate-900 rounded-2xl border border-gray-200 dark:border-slate-800 p-3.5 md:p-5 shadow-xs flex flex-col gap-3">
            ...
            <div className="flex flex-wrap items-center justify-between gap-2 pb-3 border-b border-gray-100 dark:border-slate-800">
                ...
                <div className="flex items-center gap-1.5 bg-gray-50 dark:bg-slate-800 p-1 px-2.5 rounded-xl border border-gray-200 dark:border-slate-700 text-xs font-bold">
                    <span className="text-gray-500 dark:text-slate-400 font-extrabold text-[11px]">🏢 건물 필터:</span>
                    <select
                        ...
                        {filteredRooms.map((room, idx) => {
                            ...
                            return (
                                <tr key={room.unitKey} className="hover:bg-blue-50/40 dark:hover:bg-slate-800/50 transition">
                                    {/* 순위 & 호실명 */}
                                    <td className="py-3 px-3 font-black text-gray-900 dark:text-slate-100 flex items-center gap-2">
                                        ...
                                        <span className="text-sm font-black text-blue-900 dark:text-blue-400">{room.roomName}</span>
                                    </td>

                                    {/* 소속 숙소 */}
                                    <td className="py-3 px-3 font-bold text-gray-600 dark:text-slate-300">
                                        {room.propName}
                                    </td>

                                    {/* 매출액 */}
                                    <td className="py-3 px-3 text-right font-black text-gray-900 dark:text-slate-100 font-mono text-sm">
                                        ₩{room.totalRevenue.toLocaleString()}
                                    </td>

                                    {/* 판매 박수 vs 공실 박수 */}
                                    <td className="py-3 px-3 text-center font-bold">
                                        <span className="text-blue-700 dark:text-blue-400 font-black">{room.totalNights}박 판매</span>
                                        <span className="text-gray-300 dark:text-slate-600 mx-1">/</span>
                                        <span className={room.vacantNights > 0 ? 'text-amber-600 dark:text-amber-400 font-extrabold' : 'text-gray-400 dark:text-slate-500'}>
                                    ...
                                    <td className="py-3 px-3 text-center">
                                        <div className="flex items-center justify-center gap-1.5">
                                            ...
                                            <span className="font-black text-gray-800 dark:text-slate-200 text-[11px] w-9 text-right">
                                                {room.occupancyRate}%
                                            </span>
                                        </div>
                                    </td>
                                    ...
                                    <td className="py-3 px-3 text-right relative">
                                        ...
                                        {/* 요일별 3분류 팝오버 말풍선 */}
                                        {isAdrOpen && d && (
                                            <div className="absolute right-3 top-11 z-30 bg-white dark:bg-slate-900 rounded-xl shadow-xl border border-purple-200 dark:border-purple-800 p-3 min-w-[210px] text-left flex flex-col gap-1.5 animate-in fade-in zoom-in-95 duration-150">
                                                <div className="flex items-center justify-between pb-1.5 border-b border-gray-100 dark:border-slate-800 text-[11px] font-black text-gray-900 dark:text-slate-100">
                                                ...
                                                <div className="flex flex-col gap-1 text-[11px]">
                                                    <div className="flex items-center justify-between">
                                                        <span className="text-gray-600 dark:text-slate-400 font-bold flex items-center gap-1">
                                                        ...
                                                        <span className="font-black text-gray-900 dark:text-slate-100 font-mono">
                                                            ...
                                                            <span className="text-gray-400 dark:text-slate-500 font-normal text-[10px] ml-1">({d.weekdayNights}박)</span>
                                                        </span>
                                                    </div>
                                                    ...
                                                    <div className="flex items-center justify-between">
                                                        ...
                                                        <span className="font-black text-amber-900 dark:text-amber-300 font-mono">
                                                            ...
                                                            <span className="text-gray-400 dark:text-slate-500 font-normal text-[10px] ml-1">({d.weekendNights}박)</span>
                                                        </span>
                                                    </div>
                                                    ...
                                                    <div className="flex items-center justify-between">
                                                        ...
                                                        <span className="font-black text-blue-900 dark:text-blue-300 font-mono">
                                                            ...
                                                            <span className="text-gray-400 dark:text-slate-500 font-normal text-[10px] ml-1">({d.sundayNights}박)</span>
                                                        </span>
                                                    </div>
                                                </div>
                                            </div>
            ...
            <div className="grid grid-cols-1 gap-2.5 md:hidden">
                {filteredRooms.map((room, idx) => {
                    ...
                    return (
                        <div
                            key={room.unitKey}
                            className="bg-gray-50/80 dark:bg-slate-800/80 rounded-xl border border-gray-200 dark:border-slate-700 p-3 flex flex-col gap-2 shadow-2xs"
                        >
                            <div className="flex items-center justify-between">
                                <div className="flex items-center gap-2">
                                    <span className={`w-5 h-5 rounded-full flex items-center justify-center text-[10px] font-black shrink-0 ${
                                    ...
                                    <span className="font-black text-xs text-blue-900 dark:text-blue-400">{room.roomName}</span>
                                    <span className="text-[10px] font-bold text-gray-500 dark:text-slate-400">({room.propName})</span>
                                </div>
                            ...
                            <div className="grid grid-cols-3 gap-1 pt-1.5 border-t border-gray-200/60 dark:border-slate-700/60 text-[11px]">
                                <div>
                                    <span className="text-gray-400 dark:text-slate-500 text-[10px] block">가동률</span>
                                    <span className="font-bold text-indigo-700 dark:text-indigo-400">{room.occupancyRate}%</span>
                                </div>
                                <div>
                                    <span className="text-gray-400 dark:text-slate-500 text-[10px] block">판매/공실</span>
                                    <span className="font-bold text-blue-700 dark:text-blue-400">{room.totalNights}박</span>
                                    <span className="text-gray-400 dark:text-slate-500 text-[10px] ml-1">({room.vacantNights}공실)</span>
                                </div>
                                <div className="text-right relative">
                                    <span className="text-gray-400 dark:text-slate-500 text-[10px] block">1박 단가</span>
                                    <div className="flex items-center justify-end gap-1">
                                        <span className="font-black text-purple-700 dark:text-purple-400">₩{room.adr.toLocaleString()}</span>
                                        <button
                                            type="button"
                                            onClick={() => setOpenAdrUnitKey(isAdrOpen ? null : room.unitKey)}
                                            className="text-[9px] font-black text-purple-600 dark:text-purple-300 bg-purple-50 dark:bg-purple-950/40 px-1 py-0.2 rounded border border-purple-200 dark:border-purple-800"
                                        >
                                            📊
                                        </button>
                                    </div>
                            ...
                            {/* 모바일 요일별 단가 펼침 */}
                            {isAdrOpen && d && (
                                <div className="mt-1 p-2 bg-white dark:bg-slate-900 rounded-lg border border-purple-200 dark:border-purple-800 flex flex-col gap-1 text-[10.5px]">
                                    <div className="flex justify-between">
                                        <span className="text-gray-600 dark:text-slate-400 font-bold">💼 월~목 (평일):</span>
                                        <span className="font-black font-mono dark:text-slate-100">{d.weekdayAdr > 0 ? `₩${d.weekdayAdr.toLocaleString()}` : '-'} ({d.weekdayNights}박)</span>
                                    </div>
                                    <div className="flex justify-between">
                                        <span className="text-amber-700 dark:text-amber-400 font-bold">🔥 금~토 (주말):</span>
                                        <span className="font-black font-mono text-amber-900 dark:text-amber-300">{d.weekendAdr > 0 ? `₩${d.weekendAdr.toLocaleString()}` : '-'} ({d.weekendNights}박)</span>
                                    </div>
                                    <div className="flex justify-between">
                                        <span className="text-blue-700 dark:text-blue-400 font-bold">🌤️ 일요일:</span>
                                        <span className="font-black font-mono text-blue-900 dark:text-blue-300">{d.sundayAdr > 0 ? `₩${d.sundayAdr.toLocaleString()}` : '-'} ({d.sundayNights}박)</span>
                                    </div>
                                </div>
                            )}
                        </div>
