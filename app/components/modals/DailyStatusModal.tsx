...
export default function DailyStatusModal({
    ...
    return (
        <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-sm flex items-center justify-center p-2.5 md:p-6 animate-fadeIn">
            <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-2xl w-full max-w-4xl max-h-[90vh] flex flex-col overflow-hidden border border-gray-200 dark:border-slate-800">
                ...
                <div className="daily-modal-tabs-wrap p-2.5 bg-gray-50 dark:bg-slate-900 border-b border-gray-200 dark:border-slate-800 shrink-0">
                    <DailyModalTabs
                        activeTab={mobileTab}
                ...
                {/* 3. 모달 본문 영역 */}
                <div className="flex-1 overflow-y-auto p-3 md:p-4 bg-gray-100 dark:bg-slate-950 min-h-0">

                    {/* 🖥️ PC 전용: 정확한 50:50 좌우 반반 분할 뷰 */}
                    <div className="daily-modal-desktop">

                        {/* 좌측 50%: 체크인 컬럼 */}
                        <div className="daily-modal-col-half flex flex-col gap-2.5 bg-white dark:bg-slate-900 p-3.5 rounded-xl border border-gray-200 dark:border-slate-800 shadow-sm">
                            <div className="flex items-center justify-between pb-2 border-b-2 border-blue-500">
                                <span className="font-black text-sm text-blue-900 dark:text-blue-300 flex items-center gap-1">
                                    <span>📥</span> 오늘 체크인
                                </span>
                                <span className="text-xs font-black bg-blue-100 dark:bg-blue-950/80 text-blue-800 dark:text-blue-300 border border-blue-200 dark:border-blue-800 px-2.5 py-0.5 rounded-full">
                            ...
                            <div className="flex flex-col gap-2 max-h-[58vh] overflow-y-auto pr-1">
                                {checkInBookings.length === 0 ? (
                                    <div className="text-center py-12 text-gray-400 dark:text-slate-500 font-bold text-xs bg-gray-50 dark:bg-slate-800/40 rounded-lg border border-dashed border-gray-300 dark:border-slate-700">
                                        체크인 예정 예약이 없습니다.
                                    </div>
                                ) : (
                        ...
                        <div className="daily-modal-col-half flex flex-col gap-2.5 bg-white dark:bg-slate-900 p-3.5 rounded-xl border border-gray-200 dark:border-slate-800 shadow-sm">
                            <div className="flex items-center justify-between pb-2 border-b-2 border-orange-500">
                                <span className="font-black text-sm text-orange-900 dark:text-amber-300 flex items-center gap-1">
                                    <span>📤</span> 오늘 체크아웃
                                </span>
                                <span className="text-xs font-black bg-orange-100 dark:bg-amber-950/80 text-orange-800 dark:text-amber-300 border border-orange-200 dark:border-amber-800 px-2.5 py-0.5 rounded-full">
                    ...
                    <div className="daily-modal-mobile">
                        {mobileTab === 'checkIn' ? (
                            checkInBookings.length === 0 ? (
                                <div className="text-center py-12 text-gray-400 dark:text-slate-500 font-bold text-xs bg-white dark:bg-slate-900 rounded-xl border border-dashed border-gray-300 dark:border-slate-700">
                                    체크인 예정 예약이 없습니다.
                                </div>
                            ) : (
                                sortedCheckInBookings.map((b) => (
                ...
                {/* 4. 하단 닫기 바 */}
                <div className="p-3 bg-white dark:bg-slate-900 border-t border-gray-200 dark:border-slate-800 flex justify-end shrink-0">
                    <button
                        type="button"
                        onClick={onClose}
                        className="px-4 py-1.5 bg-gray-800 hover:bg-black dark:bg-slate-800 dark:hover:bg-slate-700 text-white font-black text-xs rounded-lg transition cursor-pointer"
                    >
                        닫기
                    </button>
                </div>
